...
void
PeleCAmr::writeBasePlotFile(const int step)
{
  int base_plot_int;
  amrex::ParmParse pp("pelec");
  pp.query("base_plot_int", base_plot_int);

  if (base_plot_int == 0) {
    return;
  }	  

  if (!(step % base_plot_int == 0)) {
    return;
  }

  BL_PROFILE("PeleCAmr::writeBasePlotFile()");
  ...
  std::string base_plot_name;
  pp.query("base_plot_name", base_plot_name);
  const std::string& pltfile =
    amrex::Concatenate(base_plot_name, level_steps[0], file_name_digits);

  if (verbose > 0) {
    amrex::Print() << "BASE PLOTFILE: file = " << pltfile << '\n';
  }

  if ((record_run_info == 0) && amrex::ParallelDescriptor::IOProcessor()) {
    runlog << "BASE PLOTFILE: file = " << pltfile << '\n';
  }
...
#endif
#endif
  writeBasePlotFileDoit(pltfile, true, write_hdf5_plots, hdf5_compression); 
}
  ...
  amrex::Vector<std::string>& plt_var_names)
{
  ...
  amrex::ParmParse pp("pelec");
  amrex::Vector<std::string> base_plot_var_names;
  if (pp.contains("base_plot_vars"))
   {
  	std::string nm;
   	int nDrvPltVars = pp.countval("base_plot_vars");
  	for (int i = 0; i < nDrvPltVars; i++)
        {
           pp.get("base_plot_vars", nm, i);
           base_plot_var_names.push_back(nm);
        }
   }
  else
  {
  	std::string nm = "density";
  	base_plot_var_names.push_back(nm);
  	nm = "pressure";
  	base_plot_var_names.push_back(nm);
  }
  ...
  for (int typ = 0; typ < desc_lst.size(); typ++) {
    for (int comp = 0; comp < desc_lst[typ].nComp(); comp++) {
      for (int l = 0; l < base_plot_var_names.size(); l++) {   
        if (
          amrex::Amr::isStatePlotVar(desc_lst[typ].name(comp)) &&
          desc_lst[typ].getType() == amrex::IndexType::TheCellType()) {

	  if(base_plot_var_names[l].compare(desc_lst[typ].name(comp)) == 0)
          {
            plot_var_map.push_back(std::pair<int, int>(typ, comp));
          }
        }
  ...
  if (regular) {
    for (const auto& it : dlist) {
      for (int l = 0; l < base_plot_var_names.size(); l++) {
        if (amrex::Amr::isDerivePlotVar(it.name())) {
	  if(base_plot_var_names[l].compare(it.name()) == 0)
          {
            derive_names.push_back(it.name());
            num_derive += it.numDerive();
          }
  ...
  const int nlevels = 1;//finestLevel() + 1;
  for (int lev = 0; lev < nlevels; ++lev) {
...
void
PeleCAmr::writeBasePlotFileDoit(
  const std::string& pltfile,
  const bool regular,
  const bool write_hdf5_plots,
  const std::string& hdf5_compression)
{
  ...
  amrex::Vector<std::string> plt_var_names;
  amrex::Vector<std::unique_ptr<amrex::MultiFab>> plotMFs(nlevels);
  constructBasePlotMF(regular, plotMFs, plt_var_names);
  ...
  for (int lev = 0; lev < nlevels; ++lev) {
    istep[lev] = levelSteps(lev);
    geoms.push_back(getLevel(lev).Geom());
  }
  ...
  if (write_hdf5_plots) {
    ...
    (void)hdf5_compression; // Avoid unused warning
    amrex::WriteMultiLevelPlotfile(
      pltfile, nlevels, plotMFs_constvec, plt_var_names, geoms, cur_time,
      istep, refRatio());
    //amrex::WriteMultiLevelPlotfile(
     // pltfile, nlevels, plotMFs_constvec, plt_var_names, Geom(), cur_time,
     // istep, refRatio());
  ...
  if (verbose > 0) {
    ...
    amrex::ParallelDescriptor::ReduceRealMax(dPlotFileTime, IOProc);
    if (regular) {
      amrex::Print() << "Write base plotfile time = " << dPlotFileTime << "  seconds"
                     << "\n\n";
    } else {
      amrex::Print() << "Write small plotfile time = " << dPlotFileTime
                     << "  seconds"
