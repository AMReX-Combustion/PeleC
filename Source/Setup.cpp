...
PeleC::variableSetUp()
{
...
#endif
    else { amrex::Abort("Invalid chem_integrator choice."); }
  ...
  NumAdv = 0;
    ...
    desc_lst.addDescriptor(
      ...
      amrex::StateDescriptor::Point, ngrow_state, NVAR, interp,
    ...
    derive_lst.add(
      ...
      the_same_box);
    derive_lst.addComponent(
      "particle_density", desc_lst, State_Type, Density, 1);
#endif
    ...
    if (do_mms) {
      derive_lst.add(
        ...
        the_same_box);
      derive_lst.addComponent(
        "rhommserror", desc_lst, State_Type, Density, NVAR);
