...
! :::
  ...
  subroutine consup( &
    ...
    do i = lo(1),hi(1)+1
       flux(i,UMX) = flux(i,UMX) + area(i) * q1(i,GDPRES)
    enddo
    ...
    if (verbose .eq. 1) then
       ...
       do i = lo(1), hi(1)
          ...
          mass_added_flux = mass_added_flux + ( flux(i,URHO ) - flux(i+1,URHO ) )*dt
          xmom_added_flux = xmom_added_flux + ( flux(i,UMX  ) - flux(i+1,UMX  ) )*dt
          ymom_added_flux = ymom_added_flux + ( flux(i,UMY  ) - flux(i+1,UMY  ) )*dt
          zmom_added_flux = zmom_added_flux + ( flux(i,UMZ  ) - flux(i+1,UMZ  ) )*dt
          E_added_flux    = E_added_flux    + ( flux(i,UEDEN) - flux(i+1,UEDEN) )*dt
