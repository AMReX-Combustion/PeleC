...
#include "pelec_queries.H"
  ...
  if (do_isothermal_walls) {
    for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
      if (domlo_isothermal_temp[dir] > 0.0 and
        ...
        amrex::Abort("Cannot have isothermal wall on a BC that isn't a wall or user defined BC");
      }
      if (domhi_isothermal_temp[dir] > 0.0 and
