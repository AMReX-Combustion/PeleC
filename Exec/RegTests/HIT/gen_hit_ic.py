...
parser.add_argument(
    '-k0', help='Wave number containing highest energy', type=float,
    default=4.0)
...
# Energy spectrum
Ek = 16.0 * np.sqrt(2.0 / np.pi) * (kmag**4) / (args.k0**5) * \
...
uf[halfN - 1:0:-1, N:halfN - 1:-1,
    0] = np.conj(uf[halfN + 1:N, 1:halfN + 1, 0])

vf[N:halfN:-1, N:halfN:-1, 0] = np.conj(vf[1:halfN, 1:halfN, 0])
vf[halfN - 1:0:-1, N:halfN - 1:-1,
    0] = np.conj(vf[halfN + 1:N, 1:halfN + 1, 0])
...
wf[halfN - 1:0:-1, N:halfN - 1:-1,
    0] = np.conj(wf[halfN + 1:N, 1:halfN + 1, 0])
...
print('Energy = 1/V * int E(x,y,z) dV = 0.5/V * int (u**2 + v**2 + w**2) dx dy dz = {0:.10f} ~= 3/2'.format(
...
if args.plot:
    ...
    ax[0, 0].imshow(uo[:, :, 0].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[0, 0].set_title('Original data (x,y)')
    ax[0, 1].imshow(uo[:, 0, :].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[0, 1].set_title('Original data (x,z)')
    ax[0, 2].imshow(uo[0, :, :].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[0, 2].set_title('Original data (y,z)')

    # Filtered original data
    ax[1, 0].imshow(u[:, :, 0].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[1, 0].set_title('Filtered original data (x,y)')
    ax[1, 1].imshow(u[:, 0, :].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[1, 1].set_title('Filtered original data (x,z)')
    ax[1, 2].imshow(u[0, :, :].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[1, 2].set_title('Filtered original data (y,z)')

    # Downsampled filtered data
    ur = ur.reshape(args.N, args.N, args.N, order='F')
    ax[2, 0].imshow(ur[:, :, 0].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[2, 0].set_title('Downsampled data (x,y)')
    ax[2, 1].imshow(ur[:, 0, :].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[2, 1].set_title('Downsampled data (x,z)')
    ax[2, 2].imshow(ur[0, :, :].T, origin='lower', extent=[
                    xs, xe, xs, xe], cmap='RdBu_r', vmin=datmin, vmax=datmax)
    ax[2, 2].set_title('Downsampled data (y,z)')

    plt.savefig('hit_ic_u_{0:d}_{1:d}.png'.format(
        int(args.k0), args.N), format='png')
    ...
    plt.savefig('hit_ic_uf_{0:d}_{1:d}.png'.format(
        int(args.k0), args.N), format='png')
