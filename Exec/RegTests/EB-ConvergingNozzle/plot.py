...
def AofX(X, inletdiam, outletdiam, nozzlestart, nozzleend):
    diam = outletdiam + (inletdiam - outletdiam) * (np.array(X) - nozzleend)/(nozzlestart - nozzleend)
...
plt.clf()
plt.plot(data_10['time'], data_10['pressure-min']/1e6, 'k-', label='Inlet: Fixed P, Outlet: FOExtrap')
plt.plot(data_10['time'], data_10['pressure-max']/1e6, 'k-')
plt.plot(data_00['time'], data_00['pressure-min']/1e6, 'r-', label='Inlet: Interior P, Outlet: FOExtrap')
plt.plot(data_00['time'], data_00['pressure-max']/1e6, 'r-')
plt.plot(data_01['time'], data_01['pressure-min']/1e6, 'b-', label='Inlet: Interior P, Outlet: Characteristic')
