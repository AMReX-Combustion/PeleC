...
contains
  ...
  subroutine pc_initdata(level,time,lo,hi,nvar, &
    ...
    ! local variables
    integer :: i, j, k, n
    double precision :: x, y ,z, xysqr, xcen, ycen, delta, yy
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             !          if ((x <= 0.0175d0) .or. (x  >= 0.0225)) then
             state(i,j,k,UMY  )  = 0.0d0 
             !          else
             !            yy = x-(probhi(1)/2.0d0)-0.0025d0
             !            state(i,j,k,UMY  )  = 24.0d0*u_ref*((sin((M_PI/2.0d0)*(yy/0.0025d0)))**2.0d0)
             !          endif
             state(i,j,k,UMX  ) = 2.0d0 * eos_state % rho
