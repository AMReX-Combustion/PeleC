       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             if (sqrt(xysqr) <= (4.0d0*RC)) then
                ...
                state(i,j,k,UMX  )  = -state(i,j,k,URHO ) *(   ((y-ycen)*C/(RCsqr))*exp(-xysqr/(2.0d0*RCsqr)) )
                state(i,j,k,UMY  )  = state(i,j,k,URHO )*(u_ref + ((x-xcen)*C/(RCsqr))*exp(-xysqr/(2.0d0*RCsqr)))
                ...
                state(i,j,k,UMY  )  = state(i,j,k,URHO ) * u_ref
                state(i,j,k,UMX  )  = 0.0d0
