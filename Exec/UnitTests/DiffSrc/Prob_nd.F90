...
contains
  ...
  subroutine pc_initdata(level,time,lo,hi,nvar, &
    ...
    double precision :: Y(nspec), temp, u1, u2, u3
    integer :: i,j,k,n
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             temp = 450.d0               + COS((xc(1)+0.125d0)*2*M_PI)
             if (dim .gt. 1) temp = temp + COS((xc(2)-0.125d0)*2*M_PI)
             if (dim .gt. 2) temp = temp + COS((xc(3)+.0625d0)*2*M_PI)
             ...
             u1 =                    + COS((xc(1)+0.125d0)*2*M_PI)
             if (dim .gt. 1) u1 = u1 + COS((xc(2)-0.125d0)*2*M_PI)
             if (dim .gt. 2) u1 = u1 + COS((xc(3)+.0625d0)*2*M_PI)

             u2 = ZERO
             if (dim .gt. 1) then
                u2 =                    + COS((xc(1)-.1875d0)*2*M_PI) &
                     +                    COS((xc(2)-.0625d0)*2*M_PI)
                if (dim .gt. 2) u2 = u2 + COS((xc(3)-.0625d0)*2*M_PI)
             endif

             u3 = ZERO
             if (dim .gt. 2) then
                u3 =                    + COS((xc(1)+0.025d0)*2*M_PI) &
                     +                    COS((xc(2)-0.025d0)*2*M_PI) &
                     +                    COS((xc(3)+0.125d0)*2*M_PI)
             endif

             state(i,j,k,UMX) = state(i,j,k,URHO) * u1
             state(i,j,k,UMY) = state(i,j,k,URHO) * u2
             state(i,j,k,UMZ) = state(i,j,k,URHO) * u3
