...
subroutine initialize_pmf(filename)
  use pmf_module
  use chemistry_module, only : nspecies, get_species_index, spec_names
  character (len=*) :: filename
  integer :: n
  pmf_filename = filename
  call read_pmf()
  ! HACK HERE BECAUSE WE ARE USING THE PRESSURE, so nspecies + 4 instead of +3
  if (pmf_ncomp() .ne. nspecies + 4) then
     print *,'Number of dependent variables in file:',pmf_ncomp()
     print *,'Number expected:',nspecies+4
     stop 'pmf data file not compatible with current chemistry model, wrong number of species'
  endif

  if (pmf_names(1) .ne. "X" .and. pmf_names(1) .ne. "x")  stop 'pmf data file not compatible with pmf data reader, X or X must be first variable'
  if (pmf_names(2) .ne. "temp" .and. pmf_names(2) .ne. "Temp")  stop 'pmf data file not compatible with pmf data reader, temp or Temp must be second variable'
  if (pmf_names(3) .ne. "u" .and. pmf_names(3) .ne. 'x_velocity')  stop 'pmf data file not compatible with pmf data reader, u or x_velocity must be third variable'
  do n=1,nspecies
     if (n .ne. get_species_index(pmf_names(4+n)) .and. 'X('//trim(spec_names(n))//')' .ne. pmf_names(4+n)) then
        print *,'X('//trim(spec_names(n))//')'
        print *,pmf_names(4+n)
