  ...
  const amrex_real* probhi)
{
  ...
  // Discretize wavenumber space. Ideally we would read geomdata here to get 2*dx.

  // largest length scale. Assume x-dimension is much larger than y or z.
  const amrex::Real k_min = 1.0 / amrex::min<amrex::Real>(probhi[1]-problo[1], probhi[2]-problo[2]);

  // smallest length scale. Factor of 2 respects Nyquist frequency.
  const amrex::Real dx_base = (probhi[1] - problo[1]) / n_cells[1];
  const amrex::Real k_max = 1.0 / (2.0 * dx_base);
  ...
  // Moved these to the prob_parm.H file 
  // amrex::Vector<amrex::Real> xi(3);
  //amrex::Vector<amrex::Real> zeta(3);
  ...
  for (int m = 0; m < PeleC::h_prob_parm_device->turb_num_modes; m++) {
    ...
    for (int n = 0; n < PeleC::h_prob_parm_device->sampling_number; n++) {
      ...
      PeleC::h_prob_parm_device->k[0][n][m] = k_mag/k_min  * cos(theta) * sin(phi);
      PeleC::h_prob_parm_device->k[1][n][m] = k_mag/k_min * sin(theta) * sin(phi);
      PeleC::h_prob_parm_device->k[2][n][m] = k_mag/k_min * cos(phi);
