...
contains
  ...
  subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(C, name = "amrex_probinit")
    ...
    namelist /fortin/ pamb, phi_in, T_in, vn_in, pertmag
  ...
  subroutine pc_initdata(level,time,lo,hi,nvar, &
    ...
    double precision :: yl, yr, x, y, z, u(3), pert, radius, theta
    double precision :: blobx, bloby, rfront, y_offset, blobr
    double precision, allocatable :: pmf_vals(:)

    type (eos_t) :: eos_state

    if (.not. pmf_initialized) then
       call bl_error('pmf_init not called prior to pc_initdata')
    endif
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             if (phi_in .lt. 0.d0) then
                ...
                !pert = 0.0d0
                !radius = 2.8d0+sqrt(x**2.0d0  + y**2.0d0)
                !call pmf(radius+pert,radius+pert,pmf_vals,nPMF)

                radius = SQRT((x-blobx)**2+(y-bloby)**2)

                if (radius.LE.dx(1)) then
                   theta = 0.0d0
                else
                   theta = ATAN2((y-bloby)/radius,(x-blobx)/radius)
                endif
                ...
                pertmag = 0.02d0
                blobr = 0.0014d0
                !rfront = (blobr + pertmag* rfront * blobr - radius + y_offset) *100.d0
                rfront = (blobr + pertmag* rfront * blobr - radius + y_offset) 
