          ...
          for (int ii = -1; ii <= 1; ii++) {
            for (int jj = -1; jj <= 1; jj++) {
              for (int kk = -1; kk <= 1; kk++) {
                ...
                amrex::Gpu::Atomic::Add(&DC(i + ii, j + jj, k + kk, n), dM[L] * nbr * W(i + ii, j + jj, k + kk) * sum_kappa_inv);
                //DC(i + ii, j + jj, k + kk, n) += dM[L] * nbr * W(i + ii, j + jj, k + kk) * sum_kappa_inv;
              }
            }
          }
